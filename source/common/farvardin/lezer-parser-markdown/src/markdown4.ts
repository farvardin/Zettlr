...
const EmphasisSlash: DelimiterType = {resolve: "Emphasis", mark: "EmphasisMark"}
...
const DefaultInline: {[name: string]: (cx: InlineContext, next: number, pos: number) => number} = {
  ...
  Emphasis(cx, next, start) {
    if (next != 95 && next != 42 && next != 47) return -1
    let pos = start + 1
    if (next == 47) { // double slash
      if (cx.char(pos) != 47) return -1 // pas de double slash, pas d’emphasis
      pos++
    }
    ...
    // 61 = =  3D
    let delimiterType
  if (next == 95) delimiterType = EmphasisUnderscore
  else if (next == 42) delimiterType = EmphasisAsterisk
  else delimiterType = EmphasisSlash  
    
  return cx.append(new InlineDelimiter(
    delimiterType, start, pos,
